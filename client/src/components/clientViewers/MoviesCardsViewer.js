...
import { Box, Flex, Spacer, Center, Text, Square, Button, Modal, ModalOverlay, ModalContent, useDisclosure, ModalHeader, ModalCloseButton, ModalBody, ModalFooter } from '@chakra-ui/react'

function MoviesCardsViewer({
    maxNumberOfMovies=-1
}) {
    ...
    return (
        ...
        >
            { response!=="" ?
            <Center>
                <Text color='red' fontWeight='bold' fontSize="xl">{response}</Text>
            </Center>
            :
            <>
            { loading 
                ...
                <Modal isOpen={isOpen} onClose={onClose} isCentered scrollBehavior={'inside'} size={'xl'}>
                    ...
                    <ModalContent>
                        ...
                        <ModalBody>
                            ...
                            <Text color='gray.500' fontWeight='500' fontSize="sm" marginBottom='8px'>{selectedMovie.description}</Text>
                            ...
                            <Text color='gray.700' fontWeight='bold' fontSize="sm" marginBottom='2px'>TICKETS</Text>
                            <Flex wrap={'wrap'} direction={'row'}>
                            { selectedMovie.ticketLinks.map((ticketLink) => {
                                return <Text color='white' fontWeight='bold' fontSize="sm" bg={'black'} borderRadius={"5px"} mr={"5px"} padding={'3px 8px'}>{extractWebsiteName(ticketLink)}</Text>
                            })
                            }
                            </Flex>
                        </ModalBody>
                        ...
                        >
                            ...
                            <Square size="250px">
                                <Flex direction={'column'} padding={'4'} height={'100%'} width={'100%'}>
                                    <Text color='black' fontWeight='bold' fontSize="lg">{movie.name}</Text>
                                    <Text color='gray.600' fontWeight='500' fontSize="sm" marginBottom='5px'>Playing as {movie.actorRole}</Text>
                                    <Text color='gray.500' fontWeight='500' fontSize="sm" marginBottom='8px'>{movie.description.slice(0,125) + "..."}</Text>
                                    <Spacer />
                                    <Button colorScheme='teal' size='md' variant='outline' alignSelf={'flex-start'} onClick={() => {onOpen();setSelectedMovie(movie)}}>Learn more</Button>
                                </Flex>
                            </Square>
                        </Flex>
                    </Box>
